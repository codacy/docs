...
- **Any URL you stop serving needs a `redirect_maps` entry** in `mkdocs.yml`. URLs derive from file paths, so renaming, moving, splitting, or deleting a page breaks every inbound link to it. Before handing off, list the URLs the change retires and confirm each one resolves: build the site and check that `site/<old-path>/index.html` exists, since `mkdocs-redirects` writes a real file per entry. Absolute `https://docs.codacy.com/...` links inside the repo point at the same site and go stale the same way, so audit those too — nothing validates them.
- **A redirect cannot rescue a fragment.** `redirect_maps` maps a page to a page; the browser carries `#anchor` across to the destination, where it resolves only if an element with that id happens to exist. Splitting a page therefore strands every deep link into the sections that moved, and no config fixes it. Either keep the anchors reachable on the destination or say plainly, when handing off, which deep links you are retiring.
