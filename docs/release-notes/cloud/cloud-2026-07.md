...
codacy_tools_version_old: https://github.com/codacy/codacy-tools/releases/tag/9.3.9
codacy_tools_version_new: https://github.com/codacy/codacy-tools/releases/tag/9.3.61
---
...
- **Auto-Configuration Summary:** The UI now surfaces the last auto-configuration summary after an auto-config run, giving users visibility into which tools and patterns were applied. (OD-186)
...
- **AI PR Reviewer Paid Access Enforcement:** Fixed a bug that allowed the AI PR Reviewer (a paid feature) to be triggered on open-source and free-plan organizations without charge. (OD-311)

- **Auto-Config UI Refresh:** Fixed an issue where auto-configuration changes were not reflected immediately in the UI without a manual page reload. (OD-355)

- **Security Fixes:** Patched a stored XSS vulnerability in the admin project-list view via unescaped repository metadata (OD-244), a SQL injection in the MAS service (OD-310), and terminal control character injection in the Cloud CLI output (OD-309).
...
- **CodeSniffer Updated to v4:** codacy-codesniffer has been updated to support PHP CodeSniffer v4. Read more about this [here](cloud-2026-07-remove-codesniffer-packages.md) (TCE-1540)

- **Trivy Ignore List Now Respected in CI/CD:** Fixed an issue where the Trivy ignore list was not being applied during Codacy CI/CD pipeline runs. (TCE-1550)

- **codacy-checkov Config File Option:** Added support for passing a config file to codacy-checkov. (TCE-397)
...
- **codacy-cppcheck Spurious Include Warnings Suppressed:** Suppressed irrelevant "include" warnings from codacy-cppcheck that were generating noise in results. (TCE-1012)That covers all issues completed in July 2026 across the **Odin** and **TCE** teams, with projects that still have open items excluded as per the instructions. Let me know if you'd like to adjust the wording, add/remove any items, or reformat for a specific channel.
