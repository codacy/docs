---
description: What .codacy/codacy.config.json and .codacy/codacy.config.baseline.json are, how to edit them, and why the baseline file matters.
---
...
It adds whatever is new in `next` but wasn't in `base` (a newly detected language or framework), removes whatever was in `base` but isn't in `next` anymore (a language or framework that's gone), and otherwise leaves `current` untouched — so patterns you disabled stay disabled and parameters you tuned survive.
